...
#define DISK_PORT_EF 0x1f1
#define DISK_PORT_SC 0x1f2
// LBA访问硬盘有28位和48位，目前使用的是28位。专用的LBA端口只有24位，剩余4位在device寄存器的低4位中。
#define DISK_PORT_LBA_LOW 0x1f3 //8bits for low.  0-7
#define DISK_PORT_LBA_MID 0x1f4 //8bits for mid. 8-15
#define DISK_PORT_LBA_HIGH 0x1f5 //8bits for high. 16-23
#define DISK_PORT_DEVICE 0x1f6 //0-3 for LBA's 24-27. 第 4 位用来指定通道上的主盘或从盘，0 代表主盘，1 代 表从盘。第 6 位用来设置是否启用 LBA 方式，1 代表启用 LBA 模式，0 代表启用 CHS 模式。另外的两位: 第 5 位和第 7 位是固定为 1 的，称为 MBS 位，不用关注.
#define DISK_PORT_STATUS 0x1f7 //读的时候是状态机，写的时候是command
